...
// 📡 STRICT TYPE CONTRACT FOR THE PIPELINE
interface Repo {
  id: number;
  name: string;
...
export default function CompleteProductionPortfolio() {
  ...
  // 🛡️ Safe mount layout driver to escape SSR traces completely
  ...
  useEffect(() => { if (mounted) generateCaptcha(); }, [mounted]);
    ...
    <div className="relative min-h-screen bg-[#020208] text-[#b4b4b4] font-mono [cursor:none] select-none">
      ...
      <div className={`lg:pl-20 w-full max-w-7xl mx-auto px-6 md:px-12 py-6 relative z-10 space-y-32 transition-all duration-700 ${isViewObscured ? 'blur-3xl opacity-10 scale-[0.98] pointer-events-none' : ''}`}>
        <nav className="w-full flex justify-between items-center border-b border-white/[0.02] pb-4">
          ...
          <button onClick={() => handleTabNavigation('Contact')} className="ml-auto px-6 py-2.5 rounded-xl bg-zinc-950 border border-[#00ff66]/40 text-[#00ff66] font-extrabold text-[10px] tracking-widest uppercase">Let's Talk</button>
        </nav>
        ...
        <div ref={homeRef} className="scroll-mt-24">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
            <div className="space-y-6 flex-1">
              <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-[#00ff66]/10 border border-[#00ff66]/20 text-[9px] font-black text-[#00ff66] tracking-widest uppercase">// SECURITY ENCLAVE ACTIVATED</div>
              <h2 className="text-5xl sm:text-6xl md:text-7xl font-black uppercase tracking-tight text-white">Manish Rajdoot.</h2>
              <p className="text-xs text-zinc-400 max-w-md font-sans">Data Scientist & Programmer specializing in algorithmic database sequences and high-fidelity architectures.</p>
            </div>
            <div className="relative shrink-0">
              <div className="w-48 h-48 rounded-full border-2 border-[#00ff66]/40 bg-cover bg-center" style={{ backgroundImage: "url('https://api.dicebear.com/7.x/bottts/svg?seed=Ninja&backgroundColor=03030c')" }} />
            </div>
          </div>
        </div>
        ...
        <div ref={aboutRef} className="scroll-mt-24">
          <section className="p-8 rounded-2xl bg-[#04040c]/80 border border-white/[0.03] max-w-4xl space-y-4">
            <span className="text-[9px] font-black text-[#00ff66] uppercase tracking-widest">// SYSTEM_BIOGRAPHY</span>
            <h3 className="text-2xl font-black text-white uppercase">Bypassing Latency Matrix Barriers</h3>
            <p className="text-sm text-zinc-400 font-sans leading-relaxed">I am a data scientist and programmer optimizing complex database models and Next.js interfaces.</p>
          </section>
        </div>
        ...
        <div ref={skillsRef} className="scroll-mt-24">
          <section className="space-y-6">
            <span className="text-[9px] font-black text-[#00ff66] uppercase tracking-widest">// INTEL_CAPABILITIES</span>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {toolkit.map((item, idx) => (
                <div key={idx} className="p-5 bg-[#04040e]/90 border border-white/[0.03] rounded-xl">
                  ...
                  <p className="text-[11px] text-zinc-500 font-sans">{item.desc}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <div ref={projectsRef} className="scroll-mt-24">
          <section className="space-y-6">
            <span className="text-[9px] font-black text-[#00ff66] uppercase tracking-widest">// PRODUCTION_BLUEPRINTS</span>
            <h3 className="text-xl font-extrabold text-white uppercase">Live Repositories Portal</h3>
