...
export default function CompleteProductionPortfolio() {
  ...
  const projectAssetList = [
    { title: "MicroStart Platform", desc: "A robust micro SaaS automation ecosystem generating continuous daily startup business formulations, integrated programmatic toolkits, and dynamic token checkout frameworks with Stripe billing APIs.", tags: ["Next.js", "TypeScript", "Stripe"], icon: "⚡" },
    { title: "WallShift Manager", desc: "A professional low-latency wallpaper application for Windows architectures featuring system-level customization layers and responsive graphical user interface blocks.", tags: ["Electron", "JavaScript", "CSS"], icon: "🖼️" },
    { title: "Phonebook Diary PWA", desc: "Progressive Web Application platform enabling users to encrypt, manipulate, search, and store contact node parameters securely through cloud synchronized pipelines.", tags: ["React", "Firebase", "Tailwind"], icon: "📖" },
    { title: "AI Image Stream Engine", desc: "Generative cloud workspace solution executing complex background pipelines to ingest Stability AI and OpenAI configurations for processing live high-res image download arrays.", tags: ["Next.js", "OpenAI API", "Tailwind"], icon: "🔮" }
  ];
  ...
  const blogDatabase = [
    ...
    },
    {
      ...
      content: `
        ...
        <h4>3. EXTENDING SYSTEM ARCHITECTURES</h4>
        <p>When combined with lean backend streaming protocols, PL/SQL data blocks form an absolutely indestructible environment for tracking enterprise financial ledgers and large-scale data matrix audits.</p>
      `
    }
  ];
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setStatusMessage('Syncing matrix data node channels...');
    ...
    attachedFiles.forEach(file => {
      submissionPayload.append('attachments', file);
    });
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatusMessage('// LIVE TRANSFER ACCURATE: Mail packets dispatched via Resend.');
        ...
        setFormStatus('ERROR');
        setStatusMessage('// EXCEPTION BLOCK: Server gateway rejected frame formats.');
      }
    } catch {
      setFormStatus('ERROR');
    ...
    <div className="relative min-h-screen bg-[#020208] text-[#b4b4b4] font-mono [cursor:none] select-none">
      ...
      <div className={`lg:pl-20 w-full max-w-7xl mx-auto px-6 md:px-12 py-6 relative z-10 space-y-32 transition-all duration-700 ${isViewObscured ? 'blur-3xl opacity-10 scale-[0.98] pointer-events-none' : ''}`}>
          ...
          <div className="flex space-x-8 text-[11px] font-bold tracking-[0.2em] uppercase text-zinc-400 hidden md:flex">
            {navLinks.map((link) => (
              <span 
                ...
                className={`cursor-pointer transition-all duration-300 ${activeTab === link ? 'text-white border-b-2 border-[#00ff66] pb-2 shadow-[0_4px_10px_rgba(0,255,102,0.2)]' : 'hover:text-white'}`}
              >
                {link}
              </span>
            ))}
        ...
        <div ref={homeRef} className="scroll-mt-24">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
            <div className="space-y-6 flex-1">
              ...
              <h2 className="text-5xl sm:text-6xl md:text-7xl font-black uppercase tracking-tight leading-none text-white drop-shadow-[0_0_30px_rgba(0,255,102,0.15)]">Manish Rajdoot.</h2>
            ...
            {/* UPGRADED DIGITAL PROFILE MOUNT WITH CARTOON NINJA AVATAR */}
            <div className="relative shrink-0 select-none">
              <div className="absolute -inset-4 rounded-full bg-gradient-to-r from-[#00ff66] to-emerald-500 opacity-20 blur-xl animate-pulse" />
              <div 
                className="w-48 h-48 rounded-full border-2 border-[#00ff66]/40 bg-cover bg-center shadow-[0_0_40px_rgba(0,255,102,0.25)] transition-transform duration-500 hover:scale-105" 
                style={{ backgroundImage: "url('https://api.dicebear.com/7.x/bottts/svg?seed=Ninja&backgroundColor=03030c')" }} 
              />
            </div>
          </div>
        </div>
        ...
        <div ref={experienceRef} className="scroll-mt-24">
          <section className="space-y-6">
            <span className="text-[9px] font-black tracking-[0.3em] text-[#00ff66] uppercase block animate-pulse">// TIMELINE_LOGS</span>
              ...
              {blogDatabase.map((post) => (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-white/[0.03] flex justify-between items-center text-[9px] font-bold text-zinc-500 relative z-10 group-hover:text-zinc-300 transition-colors">
                    <span>{post.date}</span>
                    <span className="text-[#00ff66] group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
                      READ NODE <span className="text-xs">→</span>
                    </span>
                  </div>
                </div>
              ))}
      ...
      {/* COMPACT CONTACT PORTAL MATRIX MODAL */}
      {showContactModal && (
        <div className="fixed inset-0 w-screen h-screen z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
          <div className="w-full max-w-4xl p-6 md:p-10 relative my-auto bg-[#03030c]/95 border border-[#00ff66]/20 rounded-2xl shadow-2xl">
            ...
            <div className="mb-6 space-y-1">
              <span className="text-[9px] font-black tracking-[0.3em] text-[#00ff66] bg-emerald-950/40 px-3 py-1 rounded-full uppercase inline-block animate-pulse">// SYSTEM CORE PARAMETERS STREAM LAYER</span>
              <h3 className="text-2xl font-black uppercase tracking-wider text-white">Establish Direct Transmission Terminal</h3>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10" encType="multipart/form-data">
              ...
              <div className="flex flex-col space-y-1 relative">
                ...
                <div className="flex gap-2">
                  ...
                  {showCountryDropdown && (
                    <div className="absolute top-12 left-0 w-64 max-h-48 overflow-y-auto bg-[#04040e] border border-white/[0.08] rounded-xl z-[60] p-1.5 custom-scroll">
              ...
              <div className="flex flex-col space-y-1 md:col-span-2">
                ...
                <textarea required name="message" value={formDataState.message} onChange={handleInputChange} rows={3} className="bg-[#03030c]/90 border border-white/[0.05] focus:border-[#00ff66] text-xs p-3 rounded-xl text-white outline-none resize-none" placeholder="Elaborate specs or code lines here..." />
              </div>

              {/* FILE BUFFER STORAGE */}
              <div className="md:col-span-2 bg-black/40 border border-white/[0.02] p-4 rounded-xl flex flex-col space-y-2">
                <div className="flex justify-between items-center">
                ...
                {attachedFiles.length > 0 && (
                  <div className="flex flex-wrap gap-2 pt-1 max-h-20 overflow-y-auto custom-scroll">
              ...
              <div className="md:col-span-2 bg-[#050512] border border-white/[0.04] p-3.5 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4">
