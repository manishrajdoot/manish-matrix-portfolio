...
export default function ContactForm({ onClose }: ContactFormProps) {
  ...
  return (
    <div 
      className="w-full bg-[#03030c] border border-[#00ff66] rounded-2xl p-5 md:p-6 shadow-[0_0_35px_rgba(0,255,102,0.25)] relative mx-auto overflow-hidden select-none"
      style={{ maxWidth: '600px' }}
    >
      {/* Header with single clean close system control */}
      ...
      <form onSubmit={handleFormSubmit} className="space-y-3.5 text-left">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="flex flex-col space-y-1">
            ...
            <input required name="firstName" type="text" value={formDataState.firstName} onChange={handleInputChange} className="bg-[#050512] border border-white/[0.05] focus:border-[#00ff66] text-xs p-3 rounded-xl outline-none text-white w-full font-mono" placeholder="Manish" />
          </div>
          <div className="flex flex-col space-y-1">
            <label className="text-[9px] uppercase font-bold tracking-widest text-zinc-500 font-mono">Last Name *</label>
            <input required name="lastName" type="text" value={formDataState.lastName} onChange={handleInputChange} className="bg-[#050512] border border-white/[0.05] focus:border-[#00ff66] text-xs p-3 rounded-xl outline-none text-white w-full font-mono" placeholder="Rajdoot" />
          </div>
        </div>
        ...
        <div className="flex flex-col space-y-1">
          ...
          <input required name="email" type="email" value={formDataState.email} onChange={handleInputChange} className="w-full bg-[#050512] border border-white/[0.05] focus:border-[#00ff66] text-xs p-3 rounded-xl outline-none text-white font-mono" placeholder="name@domain.com" />
        </div>
        ...
        <div className="flex flex-col space-y-1 relative">
          ...
          <div className="flex gap-2">
            <div 
              ...
              className="flex items-center space-x-1.5 px-3 bg-[#050512] border border-white/[0.05] rounded-xl text-xs text-white cursor-pointer select-none min-w-[75px] justify-center"
            >
              <span>{selectedCountry.flag}</span>
              <span className="text-[10px] text-zinc-400 font-mono font-bold">{selectedCountry.code}</span>
            </div>
            <input required name="phone" type="text" value={formDataState.phone} onChange={handleInputChange} className="w-full bg-[#050512] border border-white/[0.05] focus:border-[#00ff66] text-xs p-3 rounded-xl outline-none text-white font-mono" placeholder="98765-43210" />
              ...
              <div className="absolute top-[62px] left-0 w-full sm:w-64 max-h-36 overflow-y-auto bg-[#04040e] border border-[#00ff66]/40 rounded-xl z-[60] p-1.5 shadow-[0_0_15px_rgba(0,255,102,0.2)]">
                {countryDatabase.map((c, idx) => (
                  <div 
                    ...
                    className="flex items-center justify-between p-2 hover:bg-white/5 rounded-lg text-xs text-zinc-300 cursor-pointer font-mono"
                  >
        ...
        <div className="flex flex-col space-y-1">
          ...
          <textarea required name="message" value={formDataState.message} onChange={handleInputChange} rows={2} className="w-full bg-[#050512] border border-white/[0.05] focus:border-[#00ff66] text-xs p-3 rounded-xl text-white outline-none resize-none font-mono" placeholder="Elaborate project specifications..." />
        </div>
        ...
        <div className="bg-black/40 border border-white/[0.02] p-2.5 rounded-xl flex flex-col space-y-2">
          <div className="flex justify-between items-center">
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} className="text-[9px] font-bold px-2.5 py-1 bg-zinc-900 border border-white/[0.05] text-[#00ff66] rounded-lg hover:bg-[#00ff66]/10 transition-all">+ Add Files</button>
        ...
        <div className="space-y-2 pt-1">
          <button type="submit" className="w-full py-3 rounded-xl bg-zinc-950 border border-[#00ff66]/60 hover:border-[#00ff66] text-[#00ff66] font-black text-xs tracking-widest uppercase shadow-[0_0_20px_rgba(0,255,102,0.15)] hover:shadow-[0_0_25px_rgba(0,255,102,0.35)] transition-all">
